// google-search.js  (TASK 2)
// Usage: node google-search.js "your search keyword"
// Checks the same results for kendis.io's rank, paging through Google if needed.
...
async function extractResults(page) {
  ...
  return page.evaluate(() => {
    ...
    document.querySelectorAll('#search h3').forEach((h3) => {
      ...
      items.push({ title: h3.innerText.trim(), url: link.href, displayUrl });
    });
    return items;
  });
}
...
async function main() {
  ...
  if (!keyword) {
    ...
    console.error('  node google-search.js "PI planning"');
    process.exit(1);
  }
